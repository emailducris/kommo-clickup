...
app.post('/webhook', async (req, res) => {
  ...
  if (!status) {
    console.warn('❌ Dados incompletos recebidos do Kommo:', JSON.stringify(data, null, 2));
    return res.status(200).send('Dados incompletos ou sem status');
  }

  if (status.toLowerCase() === 'demanda identificada') {
    ...
    try {
      const response = await axios.post(
        `https://api.clickup.com/api/v2/list/${CLICKUP_LIST_ID}/task`,
        {
          ...
          status: 'Open' // Altere conforme o status desejado
        },
        {
          headers: {
